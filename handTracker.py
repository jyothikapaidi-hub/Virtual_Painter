...
class HandTracker():
    def __init__(self, mode=False, maxHands=2, detectionCon=0.5, trackCon=0.5):
        ...
        self.detectionCon = detectionCon  # Ensure these are floats
        self.trackCon = trackCon

        self.mpHands = mp.solutions.hands
        self.hands = self.mpHands.Hands(
            static_image_mode=self.mode,                 # Explicitly named arguments
            max_num_hands=self.maxHands,                # Integer
            min_detection_confidence=float(self.detectionCon),  # Float
            min_tracking_confidence=float(self.trackCon)  # Float
        )
